...
lib_module = emulator.load_library(
    "example_binaries/libnative-lib.so", do_init=False)
...
def hook_code(mu, address, size, user_data):
    ...
    print('# Tracing instruction at 0x%x, instruction size = 0x%x, instruction = %s' % (
...
result = emulator.call_symbol(lib_module, 'encrypt')
